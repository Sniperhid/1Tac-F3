...
["MapClickTeleport",[f_param_mapClickTeleport_textSelect]] call BIS_fnc_showNotification;
openMap [true, false];
onMapSingleClick "f_param_mapClickTeleport_telePos = _pos; f_telePositionSelected = true";
...
if (f_param_mapClickTeleport_Height != 0) then {
	f_param_mapClickTeleport_telePos = [f_param_mapClickTeleport_telePos select 0,f_param_mapClickTeleport_telePos select 1,f_param_mapClickTeleport_Height];
};

// Move player
// If the player is in a vehicle and not HALO-ing, the complete vehicle is moved. Otherwise the player is teleported.

if (vehicle player != player && f_param_mapClickTeleport_Height == 0) then {
	(vehicle player) setPos (f_param_mapClickTeleport_telePos findEmptyPosition [0,150,typeOf (vehicle player)]);
} else {
	player setPos f_param_mapClickTeleport_telePos;
};

// Move group
// If enabled, the player's group is moved next to him

if (f_param_mapClickTeleport_GroupTeleport) then {
	sleep 0.1;
	{
		[[_x,f_param_mapClickTeleport_telePos],"f_fnc_mapClickTeleportGroup",_x] spawn BIS_fnc_MP;
...
f_param_mapClickTeleport_Used = f_param_mapClickTeleport_Used + 1;

if (f_param_mapClickTeleport_Uses == 0 || f_param_mapClickTeleport_Used < f_param_mapClickTeleport_Uses) then {
