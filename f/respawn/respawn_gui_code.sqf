fn_respawnMenuInit = {
    ...
    // Marker Type
    _i = 0;
    _control = ((findDisplay 26893) displayCtrl 26900);
    {
        _control lbAdd (_x select 1);
        _markerImg = (_x select 0);
        if (_markerImg != "") then {
            _control lbSetPicture[_i,_markerImg]; 
            _control lbSetColor[_i,[1,0,0,1]];
        };
        _i = _i +1;
    } forEach respawnMenuMarkers;
    _control lbSetCurSel 0;
    
    // Marker Colours
     _control = ((findDisplay 26893) displayCtrl 26901);
    _i = 0;
    {
        _control lbAdd (_x select 1);
        _control lbSetColor [_i,(_x select 0)];
        _i = _i +1;
    } forEach respawnMenuMarkerColours;
    _control lbSetCurSel 0;
...
fn_respawnMenuRespawnAction = {
    ...
    _markerType = lbCurSel ((findDisplay 26893) displayCtrl 26900); // type == -1 if no spawn marker
    _markerColor =lbCurSel ((findDisplay 26893) displayCtrl 26901);
    ...
    var4_markerColor = _markerColor;
    var5_markerName = _markerName;
};
