...
f_param_cacheSleep = 6; 		// The time to sleep between checking
f_param_cacheRun = true;

[_range] spawn f_fnc_cTracker;

// Start the debug tracker
if (f_param_debugMode == 1) then {
	player globalchat format ["f_fnc_cInit DBG: Starting to track %1 groups, %2 range, %3 sleep",count allGroups,_range,f_param_cacheSleep];

	[] spawn {

	// Giving the tracker a head start
	sleep (f_param_cacheSleep * 1.1);
