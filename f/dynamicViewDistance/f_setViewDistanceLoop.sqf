...
while {!isNull player} do {
	_veh = vehicle player;
	_seat = "";
	_vd = f_param_viewDistance_default;

	if (_veh != player) then {
		if (_veh isKindOf "Car") then {_vd = f_param_viewDistance_car;};
		if (_veh isKindOf "Tank") then {_vd = f_param_viewDistance_tank;};
		if (_veh isKindOf "Helicopter_Base_F") then {_vd = f_param_viewDistance_rotaryWing;};
		if (_veh isKindOf "Plane") then {_vd = f_param_viewDistance_fixedWing;};

		_seat = (assignedVehicleRole player select 0);
		if (_seat == "CARGO" && f_param_viewDistance_crewOnly) then {_vd = f_param_viewDistance_default;};
	};
