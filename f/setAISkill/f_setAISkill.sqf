...

{
    ...
    if !(_x getVariable ["f_setAISkill",false]) then {
		...
		switch (side _x) do {
			case west: {_skill = f_param_skillBLU};
			case blufor: {_skill = f_param_skillBLU};
			case east: {_skill = f_param_skillOPF};
			case opfor: {_skill = f_param_skillOPF};
			case independent: {_skill = f_param_skillRES};
			case resistance: {_skill = f_param_skillRES};
			case civilian: {_skill = f_param_skillCIV};
		};
		...
		for "_i" from 0 to 8 do {
			_skilllevel = (f_param_skillSet select _i) * _skill;
