...
if (isNil "f_param_skillRes") then
{
	f_param_skillRes =
...
f_param_skillCiv = 99;
if (!isNil "f_param_civAI") then {
	f_param_skillCiv =
	switch (f_param_civAI) do {
		case west: {f_param_skillBlu};
		case blufor: {f_param_skillBlu};
		case east: {f_param_skillOpfor};
		case opfor: {f_param_skillOpfor};
...
{
	player sideChat format ["DEBUG (f\setAISkill\f_setAISkillAD.sqf): f_skillBLU = %1",f_param_skillBlu];
	player sideChat format ["DEBUG (f\setAISkill\f_setAISkillAD.sqf): f_skillRES = %1",f_param_skillOpf];
	player sideChat format ["DEBUG (f\setAISkill\f_setAISkillAD.sqf): f_skillOPF = %1",f_param_skillRes];
	player sideChat format ["DEBUG (f\setAISkill\f_setAISkillAD.sqf): f_skillCIV = %1",f_param_skillCiv];
};
