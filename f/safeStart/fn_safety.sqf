...
switch (_this select 0) do
{
	...
	case true:
	{
		...
		if (vehicle player != player && {player in [gunner vehicle player,driver vehicle player,commander vehicle player]}) then {
			player setVariable ["f_param_safetyVeh",vehicle player];
			(player getVariable "f_param_safetyVeh") allowDamage false;

			if (isNil "f_eh_safetyVeh") then {
				f_eh_safetyVeh = (player getVariable "f_param_safetyVeh") addEventHandler["Fired", {deletevehicle (_this select 6);}];
			};
		};
	...
	default {
		...
		if !(isNull(player getVariable ["f_param_safetyVeh",objnull])) then {
			(player getVariable "f_param_safetyVeh") allowDamage true;

			if !(isNil "f_eh_safetyVeh") then {
				(player getVariable "f_param_safetyVeh") removeeventhandler ["Fired", f_eh_safetyVeh];
				f_eh_safetyVeh = nil;
			};
			player setVariable ["f_param_safetyVeh",nil];
		};
